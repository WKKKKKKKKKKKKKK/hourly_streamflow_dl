...
OUT_DIR = MTS_DIR / 'outputs' / 'spatial_generalization_eval_mts100_idx2_valtest_conservative'

VAL_START, VAL_END = config.VAL_START, config.VAL_END
TEST_START, TEST_END = config.TEST_START, config.TEST_END
...
BEST_VAL_NSE = 0.7179507274017882
BEST_TEST_KGE = 0.7211588814761928
BEST_TEST_NSE = 0.6938193376278703
...
def main() -> None:
    ...
    valid_station_metrics = station_metrics[
        station_metrics['val_score_status'].eq('ok') & station_metrics['test_score_status'].eq('ok')
    ].copy()
    excluded_station_metrics = station_metrics[
        station_metrics['val_score_status'].ne('ok') | station_metrics['test_score_status'].ne('ok')
    ].copy()
    ...
    for col in ['median_val_kge', 'median_val_nse', 'median_test_kge', 'median_test_nse']:
    ...
    lines = [
        ...
        f'Tuned validation median NSE: {BEST_VAL_NSE:.15f}',
        f'Tuned test median KGE: {BEST_TEST_KGE:.15f}',
        f'Tuned test median NSE: {BEST_TEST_NSE:.15f}',
