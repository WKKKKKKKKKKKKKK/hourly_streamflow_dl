...
EXP_DIR="/home/kongw0a/MTS_LSTM/experiment_withcursor/MTSLSTM"
cd "$EXP_DIR"

SWEEP_ID="${SWEEP_ID:-${1:-}}"
if [[ -z "${SWEEP_ID}" ]]; then
  echo "ERROR: Missing SWEEP_ID. Usage: sbatch --array=1-12%12 sweep_agent_array.sh <entity/project/sweep_id>"
  echo "Or: export SWEEP_ID=<...> and sbatch --array=1-12%12 sweep_agent_array.sh"
  exit 2
fi

# Allow passing a short sweep id (e.g. qap5q0kj). In that case we need entity+project.
export WANDB_PROJECT="${WANDB_PROJECT:-MTSLSTM_Streamflow}"
if [[ "${SWEEP_ID}" != */*/* ]]; then
