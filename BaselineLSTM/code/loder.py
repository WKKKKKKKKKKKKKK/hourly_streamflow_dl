...
def generate_batches_LSTM(dataset, target, static_df=None, lookback=365*24,
    ...
    for stn in stations:
        ...
        if static_df is None:
            ...
            x_static = np.repeat(static_df.loc[stn].values[np.newaxis, :], x_dyn.shape[0], axis=0).astype('float32')
...
class LSTMDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.static_df is None:
            ...
            x_static = np.asarray(self.static_df.loc[stn].values, dtype="float32")
