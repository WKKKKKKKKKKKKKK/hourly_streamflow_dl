...
def fig_africa_hourly(out: Path) -> str | None:
    """The hourly runoff behind the daily African score, beside a physical baseline.

    Figure 7 shows the same three catchments daily, because daily is the only resolution
    at which an African score exists. This shows what the model is actually emitting
    underneath that: 24 values per day, of which the daily figure plots only the mean.
    ...
    observation is a step, because that is genuinely all that was measured. An hourly
    curve whose 24 values average onto the observed step is making the same statement the
    KGE table makes -- the point of the panel is what the curve does BETWEEN the steps,
    which no score in this project can reach.
    ...
    routing, so its sub-daily shape is expected to be too fast regardless of skill. Its
    daily median KGE over these catchments is -0.334 against the model's +0.576, which is
    the only comparison of the two that rests on observations.
    """
    ...
    ids = [s for s in scored.station_id.astype(str) if s in set(sim.station_id.astype(str))]
    ids = sorted(set(sim.station_id.astype(str)), key=lambda s: label_of.get(s, 0.0))
    if not ids:
        return None
    ...
    # Right column: a 7-day zoom on the largest event in that window, because 2,160 hours
    # compressed into one axis hides the very structure the figure is about.
    fig, axes = plt.subplots(len(ids), 3, figsize=(11.4, 2.0 * len(ids) + 1.1),
                             gridspec_kw={"width_ratios": [2.1, 1.0, 0.95]}, squeeze=False)
    for row, sid in enumerate(ids):
        ...
        end = vol.idxmax() if vol.notna().any() else daily["date"].max()
        start = end - pd.Timedelta(days=89)
        win = g.loc[g.time.between(start, end + pd.Timedelta(hours=23))]
        peak_day = (daily.loc[daily.date.between(start, end)]
        ...
        for col, (frame, note) in enumerate(((win, "90-day window"), (zoom, "7-day zoom"))):
            ...
            ax.step(d["date"], d["obs_daily"], where="post", color=INK, lw=1.4,
                    zorder=5, label="Observed (daily, the only measurement)")
            ax.fill_between(d["date"], 0, d["obs_daily"], step="post",
                            color=GREY, alpha=0.22, lw=0, zorder=1)
            ax.plot(frame["time"], frame["ensemble_M0"] * 24, color=BLUE, lw=0.9,
                    alpha=0.85, zorder=3, label="M0 hourly (zero-shot)")
            ax.plot(frame["time"], frame["ensemble_M1"] * 24, color=ORANGE, lw=1.0,
                    zorder=4, label="M1 hourly (after African daily fine-tuning)")
            era5_peak = np.nan
            if base is not None and not base.empty:
                b = base.loc[base.time.between(frame.time.min(), frame.time.max())]
                if not b.empty:
                    ax.plot(b["time"], b["era5_land_hourly"] * 24, color=AQUA,
                            lw=0.6 if col == 0 else 0.9, alpha=0.6 if col == 0 else 0.9,
                            zorder=2, label="ERA5-Land hourly runoff (baseline)")
            ...
            # daily mean over this window is 7.7 mm/d against an observed 7.6. So the peak
            ...
            if np.isfinite(top) and top > 0:
                ...
                if np.isfinite(era5_peak) and era5_peak > top * 1.12:
                    ax.annotate(f"ERA5-Land peaks at {era5_peak:.0f} mm/d",
                                (0.985, 0.95), xycoords="axes fraction", ha="right",
                                va="top", fontsize=6.8, color=AQUA,
            ...
            if col == 0:
                ...
                ax.set_title(note, fontsize=8, loc="left", color=MUTED)
            # AutoDateLocator alone put eleven labels on a seven-day axis and they overlapped
        ...
        # Third column: the mean shape of a day, each series divided by its own mean over
        # the window. Dividing out the level is the whole point -- it separates "how much
        # water" (which the daily score already covers) from "how it is distributed inside
        # a day" (which no score in this project can reach). A flat line at 1.0 would mean
        # the hourly output carries no sub-daily information at all.
        ax = axes[row][2]
        drawn = False
        for frame, column, colour, width in (
                (win, "ensemble_M0", BLUE, 1.2), (win, "ensemble_M1", ORANGE, 1.4)):
            if frame.empty:
                continue
            prof = frame.groupby(frame.time.dt.hour)[column].mean()
            level = float(frame[column].mean())
            if not np.isfinite(level) or level <= 0:
                continue
            ax.plot(prof.index, prof.values / level, color=colour, lw=width, zorder=3)
            drawn = True
        ...
        if drawn:
            ...
            ax.axhline(1.0, color=INK, lw=0.9, ls=(0, (4, 3)), zorder=1)
            # One fixed scale for all three rows. Autoscaling each panel drew a 3% wiggle
            # at the same visual amplitude as a 150% swing, which inverts the comparison
            # the column exists to make. Panels that hug 1.0 are meant to look flat: the
            # model has no systematic time-of-day cycle, it responds to events, whereas
            # ERA5-Land's afternoon convective peak is systematic and survives averaging
            # over ninety days. The peak-to-trough ratio is printed so a flat panel still
            # carries a number.
            ax.set_yscale("log")
            ...
            for k, (colour, ratio) in enumerate(ratios):
                ax.annotate(f"x{ratio:.2f}", (0.02, 0.96 - 0.115 * k),
                            xycoords="axes fraction", ha="left", va="top",
                            fontsize=6.6, color=colour)
            ax.set_xlim(0, 23)
            ax.set_xticks([0, 6, 12, 18])
            ax.set_xticklabels(["00", "06", "12", "18"])
            ax.set_xlabel("Hour (UTC)", fontsize=7.5)
            ax.set_ylabel("Share of own mean", fontsize=7.5)
            ...
            note_cv = f"CV {cvs['M0']:.3f} -> {cvs['M1']:.3f}"
            if np.isfinite(era5_cv):
                note_cv += f", ERA5 {era5_cv:.2f}"
            ax.set_title(f"Mean shape of a day   {note_cv}", fontsize=7.6,
                         loc="left", color=MUTED)
            tidy(ax, "y")
        else:
            ax.axis("off")

    handles = [Line2D([], [], color=INK, lw=1.6, label="Observed (daily, the only measurement)"),
               Line2D([], [], color=BLUE, lw=1.6, label="M0 hourly  (zero-shot)"),
               Line2D([], [], color=ORANGE, lw=1.8, label="M1 hourly  (after African daily fine-tuning)"),
               Line2D([], [], color=AQUA, lw=1.6, label="ERA5-Land hourly runoff  (reanalysis baseline)")]
    handles.append(Line2D([], [], color=INK, lw=0.9, ls=(0, (4, 3)),
                          label="Flat day (no sub-daily shape)"))
    fig.legend(handles=handles, loc="lower center", ncol=5, bbox_to_anchor=(0.5, -0.026),
               fontsize=7.4, labelcolor=INK, frameon=False,
               columnspacing=1.1, handlelength=1.7)
    fig.suptitle("Hourly runoff under the daily African score: model output and a "
                 "reanalysis baseline",
                 fontsize=10.5, fontweight="semibold", color=INK, y=0.995)
    fig.tight_layout(h_pad=1.5, w_pad=1.4, rect=(0, 0.075, 1, 0.985))
    # The three panels are three catchments, and in two of them M1 carries MORE sub-daily
    # variation than M0 -- which is the reverse of what all 284 African basins show. Reading
    # the panels as the population would invert the finding, so the paired result is printed
    # on the figure, and read from the run's own summary rather than transcribed.
    population = ""
    ...
    stamp(fig, population +
               ...
               "median KGE is -0.334 against the model's +0.576.", y=-0.105)
