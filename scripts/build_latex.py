...
def map_caption(d: dict) -> str:
    ...
    return (
        ...
        "where zero is a real centre and the sign is the reading. Colour follows role: "
        "KGE is the aggregate score and takes its own ramp, while $r$, $\\alpha$ and "
        r"$\beta$ share one ramp because they decompose it. The same colour therefore "
        "means different things by row. Dark is good for $r$, whose ideal is the top of "
        r"its scale, and dark is over-dispersed for $\alpha$ and $\beta$, whose ideal "
        "is 1 in the middle and is marked. All twenty-four colourbar ends are pointed, "
        "and each point is earned: values lie beyond every one of them. The upper bound "
        "for $r$ is 0.95 rather than 1.0 for this reason, since 1.0 is the ceiling of a "
        "correlation and no value could ever exceed it. "
