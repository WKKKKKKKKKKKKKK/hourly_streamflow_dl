...
| the same, as a paper-shaped LaTeX report | [reports/latex/PhaseI_report.tex](reports/latex/PhaseI_report.tex) — Experiments and Results, section for section; `python -m scripts.build_latex`. The `.tex` alone will not render the figures — `python -m scripts.bundle_latex` packs it with them, flat, for Overleaf. |
